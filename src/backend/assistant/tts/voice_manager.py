...
class VoiceManager:
    ...
    def __init__(
        self, 
        piper_exe_path: Path | None = None, 
        models_dir: Path | None = None
    ):
        # Check platform to define default executable name
    ...
    def _download_piper_binary(self) -> None:
        ...
        if sys_os == "Windows":
            ...
            raise VoiceNotFoundError(f"Unsupported OS for automatic Piper download: {sys_os}")
        ...
        try:
            ...
            archive_path.unlink()
            logger.info(f"Piper binary successfully downloaded to {self.piper_exe_path}")
    ...
    def _download_voice_model(self, voice_name: str) -> None:
        ...
        logger.info(f"Voice model '{voice_name}' not found locally. Fetching from Hugging Face...")
        ...
        try:
            # 1. Download voices.json to find the exact file paths for this voice
            voices_json_path = hf_hub_download(repo_id=repo_id, filename=TTS.PIPER_VOICES_JSON_FILENAME, token=token)
            with open(voices_json_path, 'r', encoding='utf-8') as f:
                voices_data = json.load(f)
                
            if voice_name not in voices_data:
                raise VoiceNotFoundError(f"Voice '{voice_name}' not found in the Piper voices registry.")
                
            voice_info = voices_data[voice_name]
            files_to_download = list(voice_info.get("files", {}).keys())
            
            # Filter to only the .onnx and .onnx.json files (exclude model cards etc if any)
            target_files = [f for f in files_to_download if f.endswith(".onnx") or f.endswith(".json")]
            
            for file_path in target_files:
                logger.info(f"Downloading {file_path}...")
                cached_file = hf_hub_download(repo_id=repo_id, filename=file_path, token=token)
                
                # Copy from HF cache to our models_dir so Piper can find it by simple name
            ...
            logger.exception(f"Failed to download voice model '{voice_name}' from Hugging Face.")
            raise VoiceNotFoundError(f"Could not download voice '{voice_name}'.") from error
    ...
    def get_piper_executable(self) -> Path:
        ...
        if not self.piper_exe_path.exists():
            nested_path = self.piper_exe_path.parent / "piper" / self.piper_exe_path.name
            if nested_path.exists():
                self.piper_exe_path = nested_path
            else:
                raise VoiceNotFoundError(f"Piper executable still not found after download at {self.piper_exe_path}")
