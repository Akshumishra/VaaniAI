...
class AudioRecorder:
    def __init__(
        self, sample_rate: int = Audio.SAMPLE_RATE, channels: int = Audio.CHANNELS
    ):
    ...
    def record_audio(self, output_path: Path | None = None) -> Path:
        ...
        silence_threshold_frames = int(
            Recording.SILENCE_DURATION_SECONDS * self.sample_rate / Recording.CHUNK_SIZE
        )
        max_frames = int(
            Recording.MAX_DURATION_SECONDS * self.sample_rate / Recording.CHUNK_SIZE
        )
        ...
        try:
            with sd.InputStream(
                ...
                while True:
                    ...
                    if (
                        silence_frames > silence_threshold_frames
                        and len(recorded_frames) > silence_threshold_frames
                    ):
