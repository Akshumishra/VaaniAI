...
class PDFSearchSchema:
    args = [
        (
            ...
            ArgsSchema(
                type=str,
                description="The specific topic or question to search for in the PDF.",
            ),
        )
    ]
...
def execute_pdf_search(query: str) -> dict:
    ...
    is_empty = not results or (
        isinstance(results[0], str) and "No highly relevant" in results[0]
    )

    return {"status": "success" if not is_empty else "error", "results": results}
