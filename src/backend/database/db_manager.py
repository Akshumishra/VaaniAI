...
class DatabaseManager:
    ...
    def _init_db(self):
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                try:
                    conn.execute(text("ALTER TABLE document_chunks ADD COLUMN IF NOT EXISTS file_hash TEXT DEFAULT '';"))
                except Exception:
                    pass
                conn.commit()
    ...
    def save_chunks(self, metadata_list: List[dict], chunks: List[str], embeddings: np.ndarray, file_hash: str):
    ...
    def check_document_exists(self, file_hash: str) -> bool:
        ...
        with self.SessionLocal() as session:
            try:
                return session.query(DocumentChunk).filter(DocumentChunk.file_hash == file_hash).first() is not None
            except Exception as e:
                logger.error(f"Failed to check document existence: {e}")
                return False

    def search_chunks(self, query: str, query_embedding: np.ndarray, top_k: int, file_hash: str) -> List[dict]:
        """Searches the database for chunks closest to the query using Hybrid Search (Vector + FTS)."""
        with self.SessionLocal() as session:
            try:
                # 1. Vector Search
                vector_results = session.query(DocumentChunk).filter(
                ...
                lexical_results = session.query(DocumentChunk).filter(
                    DocumentChunk.file_hash == file_hash,
                ...
                sorted_chunk_ids = sorted(scores.items(), key=lambda item: item[1], reverse=True)
                top_results = [chunks_map[chunk_id] for chunk_id, _score in sorted_chunk_ids[:top_k]]
